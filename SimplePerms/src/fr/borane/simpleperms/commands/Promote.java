...
public class Promote implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String name, String[] args) {
        ...
        if(!sender.hasPermission("simpleperms.promote")){
        ...
        try {
            ResultSet resultSet = Main.getInstance().db.getConnection().prepareStatement("SELECT * FROM groups").executeQuery();
            int i = 0; while(resultSet.next()){ i++; }
            if(i > Main.getInstance().playersGroup.get(((Player)sender).getUniqueId())){
